...
impl Component for Game {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        ...
        match msg {
            ...
            Msg::OnClick((x, y)) => {
                let cell = lexicon::Cell { x, y };
                if self.cells.contains(&cell) {
                    info!("remove cell at ({}, {})", x, y);
                    self.cells.remove(&cell);
                } else {
                    info!("insert cell at ({}, {})", x, y);
            ...
            Msg::ApplyPattern(term) => {
                ...
                self.offset = (
                    (self.width as f64 / 2_f64
                        - term.width as f64
                            * self.zoom
                            * (settings.cell_size + settings.grid_width) as f64
                            / 2_f64),
        ...
        html! {
          <>
            <Board
              ...
              width={self.width}
              height={self.height}
              on_click={ctx.link().callback(move |(x, y)| {
                let cell_x = (((x as f64 - offset.0) / (zoom * (settings.cell_size + settings.grid_width))) + 0.5).floor() as i32;
                let cell_y = (((y as f64 - offset.1) / (zoom * (settings.cell_size + settings.grid_width))) + 0.5).floor() as i32;

                Msg::OnClick((cell_x,cell_y))})}
            />
            <div class="panel">
              <div class="controls">
              ...
              <div class="about">
                {"Made by "}
                <a href="https://github.com/Mengsen-W/game_of_life_rs" target="_blank" rel="noopener noreferrer">{"About"}</a>
              </div>
            </div>
            <a href="https://github.com/Mengsen-W/game_of_life_rs" target="_blank" rel="noopener noreferrer" aria-label="game_of_life_rs GitHub repository" style="position: fixed; top: 0; right: 0">
